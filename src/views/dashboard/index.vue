  ...
  <div ref="container" class="dashboard-container">
    ...
    <el-form label-position="right" label-width="60px">
      ...
      <el-form-item label="4">
        <el-input v-model="d" size="mini" type="number" clearable />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="couResults" size="mini" highlight-current-row border stripe :header-cell-style="{background: 'aliceblue'}">
      ...
      <el-table-column align="center" prop="l" :label="d" show-overflow-tooltip />
    </el-table>
      ...
      if (this.d) {
        for (let i = 0; i < 300; i++) {
          ...
          for (let j = 0; j < 300; j++) {
            ...
            for (let k = 0; k < 300; k++) {
              if ((this.a * i + this.b * j + this.c * k) > Number(this.total)) {
                break
              }
              for (let l = 0; l < 300; l++) {
                const result = this.a * i + this.b * j + this.c * k + this.d * l
                if (result === Number(this.total)) {
                  this.couResults.push({ i, j, k, l })
                  if (this.couResults.length >= 500) {
                    this.$message({ message: '凑单完成 太多了 最多显示 [' + this.couResults.length + '] 种结果', type: 'success' })
                    this.loading = false
                    return
                  }
